...
def handle_operands(results, operands, path, **kwargs):
        ...
        for op_name in operands[::-1]:
            ...
            if op_name in kwargs:
                try:
                    ...
                    logger.debug(e)
                    raise ValueError(f'Error while applying !{op_name} to {results}. {e}')
            else:
                raise ValueError(f'Unknown operand {op_name}')
            ...
            if op_type == "!":
                ...
                if isinstance(operation, type):
                    if len(results) > 1 or not isinstance(results[0], operation):
                        raise ValueError(f'Expected {operation} got {type(results[0])} for {path}\'s operand {op_name}')
...
def get(source, path, sep='.', indexes=None, **kwargs):
    if indexes is None:
        indexes = {}
    operands, paths = _split_operands(path)
    if LITERAL_TAG not in operands:
        ...
        results = [path]
    result = handle_operands(results, operands, path, **kwargs)
    return result
...
def mapper(source, *, mapping=None, mapping_path: str = None, indexes=None, **kwargs):
    ...
    if isinstance(mapping, dict):
        ...
        for k, v in mapping.items():
            key_operands, ks = _split_operands(k)
            if len(ks) > 1:
                raise ValueError(f'Only one key is allowed in {k}')
